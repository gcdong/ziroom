...
# r = requests.get(url)
# # text = r.text
# # print(re.search('共([0-9]*)页',text).group(1))
...
#         href = h.select('a')[0].attrs.get('href')
    ...
    def load_html(self, bsObj, area, address):
        ...
        for li in li_list:
            ...
            if re.search('ziroom.com/z/vr/([0-9]*).html', href) is None:
                print('自如寓')
                continue
            id = re.search('ziroom.com/z/vr/([0-9]*).html', href).group(1)
            ...
            matchd4 = re.search('距([0-9]*号.*线\)?|BRT)(.*)站(.*)米', d4)
            ...
            phase = li.select(".priceDetail .gray-6")[0].text.replace('(', '').replace(')', '')
            price_str = ''
            for price in prices:
...
a = Ziromm('http://cd.ziroom.com/z/nl/z3.html', '127.0.0.1', 'root', 'root', 'ziroom', 'chengdu')
a.main()
